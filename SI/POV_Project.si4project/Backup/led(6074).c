...
void LED_Init(void)
{
      ...
      IIC_Byte_Write(IIC_LED, LED0_ADDRESS,REG_ADDR_MODE2,0x00);
      IIC_Byte_Write(IIC_LED, LED1_ADDRESS,REG_ADDR_MODE1,0x00);
      IIC_Byte_Write(IIC_LED, LED1_ADDRESS,REG_ADDR_MODE2,0x00);
      IIC_Byte_Write(IIC_LED, LED2_ADDRESS,REG_ADDR_MODE1,0x00);
      IIC_Byte_Write(IIC_LED, LED2_ADDRESS,REG_ADDR_MODE2,0x00);
      IIC_Byte_Write(IIC_LED, LED3_ADDRESS,REG_ADDR_MODE1,0x00);
      IIC_Byte_Write(IIC_LED, LED3_ADDRESS,REG_ADDR_MODE2,0x00);
      delay_ms(5);  //delay until OSC is stable
      LED_pwm_off(LED0_ADDRESS,0,16);
	    LED_pwm_off(LED1_ADDRESS,0,16);
	    LED_pwm_off(LED2_ADDRESS,0,16);
	    LED_pwm_off(LED3_ADDRESS,0,16);
}
...
void LED_set_channel(DeviceAddress_Typedef chip_addr,uint8_t channel,Tlc59116statemode_Typedef mode)
{
	...
	//tlc59116_write_reg(chip_addr,REG_ADDR_LEDOUT0+reg_num,reg_valreg_val);
}
...
u8 LED_set_duty_singlechannel(DeviceAddress_Typedef chip_addr,uint8_t channel,uint8_t duty)
{
		...
		else
		{
		    ...
		    IIC_Byte_Write(IIC_LED, chip_addr,offset,255-duty);						
